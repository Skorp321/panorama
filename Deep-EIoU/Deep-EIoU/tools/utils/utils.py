...
def image_track(tracker, detections, embeddings, args, frame_id):
    ...
    logger.info(f"save SCT results to {args.save_path}")
    return results
...
def apply_homography_to_point(point, H):
    ...
    point_transformed = (
        point_transformed_homogeneous[:2] / point_transformed_homogeneous[2]
    )
...
def non_max_suppression(boxes, iou_threshold):
    ...
    while boxes:
        ...
        boxes = [
            box
            for box in boxes
            if box[5] != current_box[5] or iou(current_box, box) <= iou_threshold
        ]
