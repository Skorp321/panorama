...
def main():
    ...
    for frame in tqdm(cap):
        ...
        if count <= args.init_tresh:
            ...
            imgs_list = [img_copy[int(y1):int(y2), int(x1):int(x2)] for (x1,y1,x2,y2,_,_) in dets]
            cls_list = [int(cls) for (_,_,_,_,_,cls) in dets]
            embed["cls"] = cls_list
            embed["embs"] = model_reid.extract_features(imgs_list)
            embeddings = pd.DataFrame(embed)
            ...
            dets = pd.DataFrame(dets, columns=["x1", "y1", "x2", "y2", "conf", "cls"])
            dets['frame'] = np.array([count] * dets.shape[0])
            ...
            for i, row in macht_df.iterrows():
                ...
                cv2.rectangle(img_copy, (x1, y1), (x2, y2), collors[2], 1)
                cv2.putText(
                    img_copy,
                    str(int(row['id'])),
                    (x1, y1),
