...
def detect(cap, stframe, output_file_name, save_output, plot_hyperparser):
    ...
    for frame in tqdm(cap):
        ...
        if count <= args.init_tresh:
            ...
            for i, row in macht_df.iterrows():
                ...
                if row['cls']  !=  2:
                    draw_annos(collors[int(row['team'])], img_copy, img_layout_copy, x1, y1, x2, y2, row, h_point, plot_hyperparser)
                else: 
                    draw_annos(collors[3], img_copy, img_layout_copy, x1, y1, x2, y2, row, h_point, plot_hyperparser)
