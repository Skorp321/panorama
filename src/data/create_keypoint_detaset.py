...
def make_parser():
    ...
    parser.add_argument('--input_video', default='data/Swiss_vs_Slovakia-panoramic_video.mp4', type=str, help='Path to input video')
    parser.add_argument('--output_dir', default='data/interim', type=str, help='Path to output directory')
...
def process_video(args) -> None:
    ...
    anno_data = create_keypoint_detaset(args.json_path)
    i=1
    ...
    with tqdm(total=cap.get(cv2.CAP_PROP_FRAME_COUNT)) as pbar:
        while True:
            ...
            cv2.imwrite(os.path.join(args.output_dir, 'images', f'{i:04d}.jpg'), frame)
            ...
            bbox_xcyc[0][0] = bbox[0][0] + ((bbox[0][2] - bbox[0][0]) / 2.)
            bbox_xcyc[0][1] = bbox[0][1] + ((bbox[0][3] - bbox[0][1]) / 2.)
            ...
            with open(os.path.join(args.output_dir, 'annotations', f'{i:04d}.txt'), 'w') as file:
                file.write(df_str)
...
def copy_files(files: list, kind: str):
    ...
    for file in tqdm(files):
        ...
        full_anno_path = full_img_path.replace('.jpg', '.txt').replace('images', 'annotations')
        
        dst_img_dir = os.path.join(args.output_dir, 'keypoints', 'images', kind)
        os.makedirs(dst_img_dir, exist_ok=True)
        dst_anno_dir = dst_img_dir.replace('images', 'annotations')
        os.makedirs(dst_anno_dir, exist_ok=True)
        
        dst_img = os.path.join(dst_img_dir, file)
        dst_anno = dst_img.replace('.jpg', '.txt').replace('images', 'annotations')
        
        shutil.copyfile(full_img_path, dst_img)
        shutil.copyfile(full_anno_path, dst_anno)
        
                
            
def create_yolo_kaypoints_dataset(args):
...
def generate_yaml_file(args):
    ...
    yaml_file = os.path.join(args.output_dir, 'keypoints', 'keypoints.yaml')
    
    data = {}
    data['path'] = args.output_dir + 'keypoints'
    data['train'] = 'images/train'
    data['val'] = 'images/val'
    annotations, _, _ = json_to_pandas(args.json_path)
    num_keypoints = annotations['num_keypoints'][0]
...
if __name__ == '__main__':
    ...
    #create_yolo_kaypoints_dataset(args)
