...
def main():
    ...
    ## Page Setup
    tab1, tab2, tab3 = st.tabs(["Как запустить?", "Team Colors", "Настройки детекции"])
    with tab1:
    ...
    '''with tab2:
        ...
        with t1col1:
            ...
            with st.spinner('Detecting players in selected frame..'):
                results = model_players(frame, conf=0.7, half=True)
                ...
                detections_imgs_grid = [
                    [
                        detections_imgs_list[i]
                        for i in range(len(detections_imgs_list) // 2)
                    ],
                    [
                        detections_imgs_list[i]
                        for i in range(
                            len(detections_imgs_list) // 2,
                            len(detections_imgs_list),
                        )
                    ],
                ]
    ...
    with tab3:
        ...
        with bcol2:
            ...
            with bcol22t:
                show_pal = st.toggle(label="Show Annotations", value=True)
    ...
    if start_detection and not stop_detection:
        ...
        os.chdir(parent_dir)
        subprocess.run(["python", "tools/modifyde.py"])
        '''status = detect(cap, stframe, output_file_name, save_output, model_players, model_keypoints,
                        detection_hyper_params, ball_track_hyperparams, plot_hyperparser,
